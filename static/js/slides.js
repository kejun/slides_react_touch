...
$.each($('pre[data-src]'), function() {
  ...
  $.ajax({
    ...
    success: function(e) {
      ...
      var code = $('<code class="' + (pre.data('type') || ('language-' + language[pre.data('src').split('.').pop()])) + '">' + htmlEncode(e) + '</code>').appendTo(pre);
...
$('.page').on('click', function(e) {
  if (!e.target.tagName || /^a|svg$/i.test(e.target.tagName)) {
    return;
  }
  next();
})
